...
**Why RabbitMQ and not Kafka.** A few questions an hour do not need a partitioned log, a JVM
...
1. **Ack after finishing, not on delivery.** `auto_ack` loses a job the moment a worker dies
   mid-run. Manual ack after the work completes gives at-least-once — which is why the
...
4. **Long jobs must outlive the timeouts.** RabbitMQ has no consumer-side poll deadline, but
   an unacked delivery does age against `consumer_timeout` (30 minutes by default). A run
